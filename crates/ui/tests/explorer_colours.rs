...
fn every_status_letter_has_a_colour_of_its_own() {
    ...
    let files = vec![
        untracked("new.txt"),
        File::unchanged_path(at("clash.rs"), revs()).set_change_type(ChangeType::Conflicted),
        File::added(at("added.rs"), staged_revs()),
        File::unchanged_path(at("edited.rs"), staged_revs()),
