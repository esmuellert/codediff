...
fn unfolding_brings_the_children_back() {
    ...
    assert!(
        ...
        "children are visible when not folded"
    );
}
...
fn fold_state_survives_a_refresh() {
    ...
    let has_app = nodes_v1
        .iter()
        .any(|n| matches!(n, Node::File { name, .. } if name == "app.rs"));
    ...
    let has_new = nodes_v2
        .iter()
        .any(|n| matches!(n, Node::File { name, .. } if name == "new.rs"));
    assert!(
        !has_new,
        "src is still folded after refresh, new.rs is hidden"
    );

    let has_notes = nodes_v2
        .iter()
        .any(|n| matches!(n, Node::File { name, .. } if name == "notes.txt"));
...
fn list_mode_files_are_indented_under_the_heading() {
    let files: Vec<File> = ["src/deep/file.rs"].iter().map(|p| file(p)).collect();
    ...
    for node in &nodes {
        if let Node::File { indent, .. } = node {
            assert_eq!(
                indent, "  ",
                "list mode files sit under the heading: {:?}",
                indent
            );
        }
    }
...
fn list_mode_is_sorted_by_path() {
    let files: Vec<File> = ["z.rs", "a/b.rs", "a.rs"].iter().map(|p| file(p)).collect();
