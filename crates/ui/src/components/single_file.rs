...
pub fn SingleFile(scope: &mut Scope) -> Node {
    ...
    let single = match ctx.diff.as_deref() {
        Some(pipeline::diff::DiffContent::SingleFile(single)) => Some(single),
        _ => None,
    };
    let line_count = single.map(|single| single.lines.len() as u32).unwrap_or(0);
    let file_key = single.map(|single| single.file.path().as_str().to_string());
    let (view, listeners) = use_diff_viewer_navigation(scope, file_key.as_deref(), line_count);
