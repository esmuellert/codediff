...
use align::{Alignment, Comparison, DiffLayout};
use file_types::File;
...
impl Buffer {
    ...
    pub fn single_file(file: File, lines: &[&str]) -> Self {
        ...
        Self::of(BufferType::SingleFile(SingleFile::new(
            file,
            std::sync::Arc::new(lines),
        )))
    }

    /// What the pipeline compared, as something to draw.
    ///
    /// Which *kind* of buffer this becomes is decided here, from how many
    /// versions the comparison carries. It used to be decided by the pipeline,
    /// on the grounds that the last stage was the last place that knew — but
    /// the answer carries what it knew, and "which buffer kind" was always a
    /// question about drawing. See D23.
    pub fn compared(comparison: Comparison, layout: DiffLayout) -> Self {
        match comparison {
            Comparison::OneSide { file, lines } => {
                Self::of(BufferType::SingleFile(SingleFile::new(file, lines)))
            }
            Comparison::Both { file, alignment } => Self::diff(Diff::new(file, alignment), layout),
        }
    }
