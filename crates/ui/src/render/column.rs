...
pub fn draw(buf: &mut Buffer, column: Column, side: Side, rows: &[Row], painter: Painter<'_>) {
...
fn line(
    ...
    let base = theme
        .normal
        .patch(role(row.kind, side, number, is_cursor, painter));
    ...
    gutter(buf, column, y, number, number_style);

    let emphasis = base.patch(match (row.kind, side) {
        (RowKind::Unchanged, _) => Style::new(),
        (_, Side::Original) => theme.deleted_text,
        (_, Side::Modified) => theme.inserted_text,
    });
    ...
    cells::paint(
        ...
        painter.alignment.line(side, number).unwrap_or(""),
        DEFAULT_TAB_WIDTH,
        painter.left,
        Ink {
            base,
...
fn role(kind: RowKind, side: Side, number: u32, is_cursor: bool, painter: Painter<'_>) -> Style {
    let theme = painter.theme;
    if kind != RowKind::Unchanged {
        return match side {
            Side::Original => theme.deleted,
            Side::Modified => theme.inserted,
        };
    }
    if painter.alignment.moved(side, number).is_some() {
        return theme.moved;
    }
    if is_cursor {
...
fn gutter(buf: &mut Buffer, column: Column, y: u16, number: u32, style: Style) {
    let area = Rect {
        y,
        height: 1,
        ..column.gutter
    ...
    let offset = width.saturating_sub(label.chars().count() as u16);
