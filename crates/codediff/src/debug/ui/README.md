...
preview.rs            Preview shell around a production component
definition.rs         StoryDefinition and typed StoryFixture
story_host.rs         direct-story Harness and terminal hosts
stories/         definitions grouped by production component
fixtures/        reusable typed data builders
```

With no ID, `gallery_controller.rs` keeps one terminal session open while replacing the
...
`long_rust_line` for horizontal-overflow scenarios; it guarantees at least
