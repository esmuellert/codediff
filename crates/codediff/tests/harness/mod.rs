...
//! making a comparison means computing a diff, and `ui` is forbidden
...
//!
//! An [`Alignment`] owns its two files, so these helpers return a comparison
...
/// A side-by-side comparison of two texts.
pub fn diff(label: &str, before: &str, after: &str) -> DiffContent {
...
/// A side-by-side comparison from a diff the caller made, for the cases a real
...
pub fn with_diff(
    ...
    DiffContent::Diff(pipeline::file::Diff {
        file: file(label),
        alignment,
    })
}

/// A file present on both sides, shown alone.
pub fn single(label: &str, contents: &str) -> DiffContent {
    lone(file(label), contents)
}

/// A file that exists only on the modified side.
///
...
pub fn added(label: &str, contents: &str) -> DiffContent {
...
/// A session over a side-by-side comparison, in the default dark theme.
pub fn session(label: &str, before: &str, after: &str) -> TestSession {
    TestSession::new_diff(diff(label, before, after), Theme::DARK)
}
