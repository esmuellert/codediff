...
fn a_file_with_nothing_to_compare_against_gets_one_column() {
    ...
    let mut s = TestSession::new_diff(added("new.rs", "alpha\nbeta"), Theme::DARK);
    assert_eq!(
        screen(&mut s, 40, 4),
        [
            "  1 alpha                               ",
...
fn a_one_sided_file_still_scrolls() {
    ...
    let long = (1..=50)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let mut s = TestSession::new_diff(single("big.rs", &long), Theme::DARK);
    measure(&mut s);
...
fn the_added_note_is_not_styled_as_though_it_were_the_path() {
    ...
    let mut s = TestSession::new_diff(added("new.rs", "alpha"), Theme::DARK);
