...
| `cargo xtask verify-vscode [repo] [--layout side-by-side\|inline]` | VS Code and codediff render the same rows and highlight roles over real Git history | S2 |
...
`target/vscode-parity/mismatches/<layout>/`. The verifier disables the computation
...
`--layout side-by-side|inline` is passed to both producers. VS Code Inline pins
`renderSideBySide=false` and `experimental.useTrueInlineView=false`.
`--ignore-trim-whitespace true|false` is also passed to both producers, and
